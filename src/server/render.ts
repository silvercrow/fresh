...
export async function render<Data>(
  ...
  if (state[0].length > 0 || state[1].length > 0) {
    ...
    bodyHtml +=
      `<script id="__FRSH_STATE" type="application/json">${res.serialized}</script>`;

    // TODO: add instructions for generating this script
    if (res.requiresDeserializer) {
      script +=
        `const KEY="_f";export function b64decode(s){const e=atob(s),r=e.length,o=new Uint8Array(r);for(let t=0;t<r;t++)o[t]=e.charCodeAt(t);return o}function deserialize(s,e){const r=[];function o(t,i){if(typeof i=="object"&&i&&KEY in i){const n=i;if(n[KEY]==="s")return r[n.i]||(r[n.i]=e()),n.v!==void 0&&(r[n.i].value=n.v),r[n.i];if(n[KEY]==="u8a")return b64decode(n.d);if(n[KEY]==="l"){const c=n.v;return c[KEY]=n.k,c}throw new Error(\`Unknown key: \${n[KEY]}\`)}return i}return JSON.parse(s,o)}`;
    }
    if (res.hasSignals) {
